...
ClientFactory.prototype.create = function(name){
	...
	switch(name.toLowerCase()){
		case 'project':
			client = new ProjectClient(this.options);
			break;
	}
...
ProjectClient.prototype.delete = function(project_id, on_success, on_error){
	this.client.delete('projects/'+project.id, null, on_success, on_error);
	return this;
};

var client_factory = new ClientFactory({
	base_url: 'http://api.amail.io/v1/',
	basic_authentication: {
		password: '50216b94cfa580-51897234'
	}
});

var project_client = client_factory.create('project');

project_client.getAll(function(response, status_code, data){
	console.log("SUCCESS!" + JSON.stringify(data));
	data.result.forEach(function(project){
		project_client.getSingle(project.id, function(response, status_code, data){
			console.log(data);
		});
	});
