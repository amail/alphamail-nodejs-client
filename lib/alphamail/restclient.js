...
RestClient.prototype.setBaseUrl = function(base_url){
	var base_url = url.parse(base_url);
...
RestClient.prototype.get = function(path, on_complete){
	this.rawRequest('get', path, null, null, on_complete);
	return this;
};

RestClient.prototype.put = function(path, body_object, on_complete){
	this.rawRequest('put', path, body_object, null, on_complete);
	return this;
};

RestClient.prototype.post = function(path, body_object, on_complete){
	this.rawRequest('post', path, body_object, null, on_complete);
	return this;
};

RestClient.prototype.delete = function(path, on_complete){
	this.rawRequest('delete', path, null, null, on_complete);
	return this;
};

// For teh 1337's and epic customizer

RestClient.prototype.rawRequest = function(method, path, body_object, headers, on_complete, on_connection_error){
	...
	request.on('error', function(error){
		var connection_error_callback = on_connection_error || outer_scope.options.on_connection_error;
	...
	request.on('response', function(response) {
			...
			response.on('end', function() {
				...
				on_complete(response, parsed_data);
